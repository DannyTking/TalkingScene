...
namespace Walaber 
{
    ...
    static const float EPSILON_IS_EQUAL = 1e-05; /*! < if diff is less than this, values are considered equal */
    ...
    {
        return lerp( from, to, 1-cos(1.57079633f * t) );
    }	        


    static inline float slerpout(const float from, const float to, const float t)
    {
		return lerp( from, to, sin(1.57079633f *  t) );
    }    
