...
namespace Walaber
{
    ...
    /*!
     Returns current volume of this sound effect.
     */
    //----------------------------------------------------------------------------------------------
	float SoundEffectInstance::getVolume()
    {
        float returnVolume = 1.0f;
        
        if (mChannel != NULL)
        {
            mChannel->getVolume(&returnVolume);
        }
	...
	void SoundEffectInstance::setPlaybackPosition( float positionInSeconds )
	{
		if  (mChannel != NULL)
		{
			...
			if (res != FMOD_OK)
			{
				Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR setting position on sound [%s]\n", FMOD_ErrorString(res));
			}
		}
	}
	...
	unsigned int SoundEffectInstance::getPlaybackPosition()
	{
		...
		if (mChannel != NULL)
		{
			...
			if (res != FMOD_OK)
			{
				Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR getting position! [%s]\n", FMOD_ErrorString(res));
			}
		}
	...
	unsigned int SoundEffectInstance::getLength()
	{
		...
		if (res != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR getting sound length! [%s]\n", FMOD_ErrorString(res));
			return 0;
		}
	...
	unsigned int SoundEffectInstance::getData( unsigned int start, unsigned int length, uint8_t* buffer )
	{
		...
		if (res != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR getting sound format! [%s]\n", FMOD_ErrorString(res));
			return 0;
		}
		
		if (sFormat != FMOD_SOUND_FORMAT_PCM16)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR! Only PCM16 is supported for getData at this time!\n");
			return 0;
		}
		
		if (sChannels > 1)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR!  only single channel sound is supported for getData at this time!\n");
			return 0;
		}
		...
		if (res != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR locking sound! [%s]\n", FMOD_ErrorString(res));
			return 0;
		}

		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "len1[%d] len2[%d]\n", len1, len2);
		...
		if (res != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR unlocking sound! [%s]\n", FMOD_ErrorString(res));
			return 0;
		}
	...
	void SoundEffectInstance::saveToWAV( std::string& filePath, unsigned int startSample, long lengthInSamples  )
	{
		...
		if (sFormat != FMOD_SOUND_FORMAT_PCM16)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR! Only PCM16 is supported for saveToWAV at this time!\n");
			return;
		}
		
		if (sChannels > 1)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR!  only single channel sound is supported for saveToWAV at this time!\n");
			return;
		}
		...
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR opening file!\n");
		}
    ...
    void SoundEffectInstance::stopIfLooping()
    {
        if (mChannel != NULL)
		{
			FMOD_MODE mode;
	...
	void SoundEffectInstance::_initChannel()
	{
		if (mChannel == NULL)
		{
			...
			if (res != FMOD_OK)
			{
				Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR getting FMOD System object! [%s]\n", FMOD_ErrorString(res));
				return;
			}
			...
			if (res != FMOD_OK)
			{
				Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR playing sound! [%s]\n", FMOD_ErrorString(res));
				mChannel = NULL;
				return;
			}
			...
			if (res != FMOD_OK)
			{
				Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR getting frequency for channel! [%s]\n", FMOD_ErrorString(res));
			}
		}
	}
	...
	{		
		...
		if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR creating FMOD sound system! [%s]\n", FMOD_ErrorString(result));
			return false;
		}
		...
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR You are using an old version of FMOD %08x.  This program requires %08x\n", version, FMOD_VERSION);
			return false;
		}
		...
		if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR initializing FMOD sound system! [%s]\n", FMOD_ErrorString(result));
			return false;
		}
		...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "sampleRate[%d]  format[%d]  outChannels[%d] inputChannels[%d] resampleMethod[%d] bits[%d]\n", sampleRate, format, outputChannels, maxInputChannels, resampleMethod, bits);
		...
		if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR creating channel group for SFX! [%s]\n", FMOD_ErrorString(result));
			return false;
		}	
		...
		if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR creating channel group for VOICES! [%s]\n", FMOD_ErrorString(result));
			return false;
		}
		...
		if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR creating channel group for MUSIC! [%s]\n", FMOD_ErrorString(result));
			return false;
		}
	...
	void SoundManager::_initConfigFileRead( void* params ) // std::string path, char* buffer, size_t length, std::string actualPath, std::string handlerName, PropertyList userData )
	{
		...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::init - filename:%s\n", rcp->path.c_str());
				...
				for (xmlNodePtr map = kids->children; map; map = map->next)
				{
					if (xmlStrcmp(map->name, (const xmlChar*)"Mapping") == 0)
					{
						...
						if (Walaber::TextManager::stringToLanguage(XML::parseString(map, "language"), l))
						{
							...
							Walaber::Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::init - found mapping for language '%s' is folder '%s'\n",
	...
	{
		// load the file first...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::addSound( %d, %s )\n", group, filename.c_str());
	...
	void SoundManager::_loadSound( int groupID, int assetID, CallbackPtr callback, const PropertyList& plist  )
	{
		...
		if (it == mSoundGroups.end())
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "_loadSound ERROR - no such group: %d\n", groupID);
			return;
		}

		if ((assetID < 0) || (assetID >= it->second.liveFiles.size()))
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "_loadSound ERROR - assetID %d not within range [0,%d]\n", assetID, (int)it->second.liveFiles.size());
			return;
		}
		...
		{
			PropertyList plist_;
			plist_.setValueForKey("group", Property( groupID ));
			plist_.setValueForKey("id", Property( assetID ));
			plist_.setValueForKey("loop", Property( it->second.liveFiles[assetID]._loop ? 1 : 0 ));
			plist_.setValueForKey("filename", Property( it->second.liveFiles[assetID]._sourceFile ) );
			...
			if (li != mFolderMap.end())
			{
				...
				plist_.setValueForKey("is_loc_test", Property(1));
			}
			...
			FileManager::getInstancePtr()->readFile( filename, callback_, plist_ );
		}
	}
	...
	void SoundManager::_loadSoundFileRead( void* params ) //std::string path, char* buffer, size_t length, std::string actualPath, std::string handlerName, PropertyList userData )
	{
		...
		if (it == mSoundGroups.end())
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "_addSoundFileRead ERROR - no such group: %d\n", group);
			return;
		}
		
		if ((id < 0) || (id >= it->second.liveFiles.size()))
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "_addSoundFileRead ERROR - assetID %d not within range [0,%d]\n", id, (int)it->second.liveFiles.size());
			return;
		}
		...
		if (!sr)
		{
			Walaber::Logger::printf("Walaber-Sound", Logger::SV_ERROR, " --> sound was for group[%d] asset[%d]\n", group, id);
			sr = SoundResourcePtr();
		}
		...
		for (SoundInstanceCallbackMap::iterator it_ca = its.first; it_ca != its.second; it_ca++)
		{
			...
			if (it_ca->second.callback)
			{
				...
				{
                    SoundEffectInstanceLoadedParameters params_(group, id, SoundEffectInstancePtr(new SoundEffectInstance( it->second.liveFiles[id]._sound, mGroupMusic, it->second.liveFiles[id]._baseVol, it->second.liveFiles[id]._subtitleText )), it_ca->second.plist);
					it_ca->second.callback->invoke(&params_);
				}
				else
				{
					bool isVO = it->second.liveFiles[id]._isVO;
					
                    SoundEffectInstanceLoadedParameters params_(group, id, SoundEffectInstancePtr(new SoundEffectInstance( it->second.liveFiles[id]._sound, (isVO ? mGroupVO : mGroupSFX), it->second.liveFiles[id]._baseVol, it->second.liveFiles[id]._subtitleText )), it_ca->second.plist );
					it_ca->second.callback->invoke(&params_);
				}
			}
		}	
    ...
    SoundResourcePtr SoundManager::_loadSoundResource( FileManager::ReadFileCallbackParameters* rfc, bool loop )
	{
		...
		if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR loading sound {%s} -- [%s]\n", 
		...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "got sound (%s) : mem usage %d  ", rfc->path.c_str(), memused );
		
		switch (fmt)
		{
			case FMOD_SOUND_FORMAT_PCM16:
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "PCM16 ");
				break;
				
			case FMOD_SOUND_FORMAT_PCM24:
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "PCM24 ");
				break;
				
			case FMOD_SOUND_FORMAT_PCM8:
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "PCM8 ");
				break;
				
			case FMOD_SOUND_FORMAT_MPEG:
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "MPEG ");
				break;
				
			case FMOD_SOUND_FORMAT_PCMFLOAT:
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "PCMFLOAT ");
				break;
				
			default:
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "FORMAT??? ");
				break;
		}
		
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, " %dCHAN %dBIT ", channels, bits);
		...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, " %4.2fHz, %4.2fVOL %4.2fPAN %dPRIORITY\n", freq, vol, pan, priority);
#endif
	...
	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	int SoundManager::playSoundFromGroup( int group, float volume, float pitch )
	{
		PropertyList plist;
		...
		return newSoundFromGroup(group, callback, plist);
	}
	
    // --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	std::string SoundManager::getFilenameForSoundInGroup( const int group, const int soundID ) const
    {
        // get a random sound from the group.
		SoundGroupMap::const_iterator it = mSoundGroups.find( group );
		
		if (it == mSoundGroups.end() || !it->second.liveFiles.size())
			return "";
		
        if ((soundID < 0) || (soundID >= it->second.liveFiles.size()))
			return "";
        
        return StringHelper::intToStr( soundID ) + std::string(" - ") + it->second.liveFiles[soundID]._sourceFile;
    }

	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	int SoundManager::newSoundFromGroup( int group, CallbackPtr callback, const PropertyList& plist )
	{
		// get a random sound from the group.
		SoundGroupMap::iterator it = mSoundGroups.find( group );
	...
	void SoundManager::_loadSoundFileDirectRead(void *params)
	{
		...
		for (SoundFileCallbackMap::iterator it_ca = its.first; it_ca != its.second; it_ca++)
		{
			...
			if (it_ca->second.callback)
			{
				SoundFileInstanceLoadedParameters soundFileParameters(origFile, SoundEffectInstancePtr(new SoundEffectInstance( si._sound, (isVO ? mGroupVO : mGroupSFX), si._baseVol, si._subtitleText )), it_ca->second.plist );
				soundFileParameters.result = result;
				
				it_ca->second.callback->invoke(&soundFileParameters);
			}
		}	
	...
	*/
	...
	void SoundManager::freeGroup( int groupID, bool force )
	{
		...
		if (it == mSoundGroups.end())
		{
			Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::freeGroup( %d ) ERROR -> group does not exist.\n", groupID );
		...
		if (!force)
		{
			if (mInvulnerableSoundGroups.find( it->first ) != mInvulnerableSoundGroups.end())
			{
				...
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::freeGroup() -- sound group %d is set to always stay in memory, skipping it!\n", it->first);
		...
		if (allClear)
		{
			...
			Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::freeGroup() ==> removing group [%d] from memory...\n", it->first);

			for (unsigned int i = 0; i < it->second.liveFiles.size(); i++)
			{
				Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "  -> sound index [%d] removed\n", i);
	...
	bool SoundManager::systemMusicIsPlaying()
	{
		...
		if (res != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR checking for system music: [%s]\n", FMOD_ErrorString(res));
		}
#endif
	...
	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	// --=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=
	bool SoundManager::updateMusicMuteStatus()
	{
        bool muted = false;
		if (mGroupMusic)
		{
            muted = systemMusicIsPlaying();
			mGroupMusic->setMute( muted );
		}
        return muted;
	}
...
//	}
    ...
    void SoundManager::switchThemeMusicTo(int groupID)
    {
        for (int j=0; j< mHubMusicGroups.size(); j++)
        {
            if (groupID == mHubMusicGroups[j])
            {
                mHubMusicChannels[j]->setVolume(0.7f);
                mCurrentMusicGroup = groupID;
                mCurrentIndex = j;
            }
            else
            {
                mHubMusicChannels[j]->setVolume(0.0f);
            }
        }
    }
    ...
    /*
     This function dosn't work on Android and was not proven to be stable yet since we have never
     used this function. Use "playMusicFromGroup(int groupID)" instead of this.
     - Andrew
     */
    bool SoundManager::playHubMusicFromGroup(int groupID, float fadeLength)
    {
        ...
        if (playMusic)
        {
            for (int i=0; i< mHubMusicChannels.size(); i++)
            {
                ...
                {
                    // In WMM, don't ever pause theme tracks.
            ...
            if (it != mMusicGroups.end() && it->second.liveFiles.size())
            {
                ...
                if (result != FMOD_OK)
                {
                    Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR loading streamed audio file {%s} - [%s]\n", it->second.liveFiles[it->second.currentTrackID].c_str(), FMOD_ErrorString(result));
                    mHubMusicSound = NULL;
                    ret = false;
                }
                else
	...
	bool SoundManager::playSpecificMusicFromGroup( int groupID, int trackID )
	{
        ...
        {       
            ...
            {            
                ...
                {
                    ret = false;
					Logger::printf("Walaber-Sound", Logger::SV_INFO, "not in live music group: %i  track id: %i\n", groupID, trackID );
                }
                else
                {
                    Logger::printf("Walaber-Sound", Logger::SV_INFO, "music track: %s\n", it->second.liveFiles[trackID].c_str());
            ...
            {
                ret = false;
				Logger::printf("Walaber-Sound", Logger::SV_INFO, "Muisc group does not exist group: %i\n", groupID );
            }
        }
		...
		for (SoundGroupMap::iterator it = mSoundGroups.begin(); it != mSoundGroups.end(); ++it)
		{
			...
			for (SoundList::iterator j = it->second.liveFiles.begin(); j != it->second.liveFiles.end(); ++j)
			{
				if ((*j)._isVO)
				{
					...
					Walaber::Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "Subtitle found for [%s] --> '%s'\n",
	...
	void SoundManager::debugLogMemory()
	{
		...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SoundManager::debugLogMemory--- allocated[%d]bytes, max allocated[%d]bytes\n", alloced, maxalloced);
		...
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "---------------------------\n");
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "NAME\tBYTES:\n");
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "---------------------------\n");
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SYSTEM\t%d\n", details.system);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "OTHER\t%d\n", details.other);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "PLUGINS\t%d\n", details.plugins);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "CODEC\t%d\n", details.codec);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "CHANNEL\t%d\n", details.channel);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "CHANNEL GROUP\t%d\n", details.channelgroup);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "FILE\t%d\n", details.file);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "SOUND\t%d\n", details.sound);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "STREAM BUFFER\t%d\n", details.streambuffer);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "TOTAL\t%d\n", memused);
		Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "---------------------------\n");
	}
    ...
    void SoundManager::addUnderWaterMusicFilter()
    {
        ...
        if (result != FMOD_OK)
        {
            Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR Getting Default Reverb properties [%s]\n", FMOD_ErrorString(result));
            return;
        }
        else
        {
            ...
            if (result != FMOD_OK)
            {
                Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR Adding Underwater sound filter [%s]\n", FMOD_ErrorString(result));
                return;
            }
            else
            {
                Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "Added Underwater sound filter \n");
            }
    ...
    void SoundManager::removeUnderWaterMusicFilter()
    {
        ...
        if (result != FMOD_OK)
		{
			Logger::printf("Walaber-Sound", Logger::SV_ERROR, "ERROR Removing Underwater sound filter [%s]\n", FMOD_ErrorString(result));
            return;
		}
		else
        {
            Logger::printf("Walaber-Sound", Logger::SV_DEBUG, "Removed Underwater sound filter \n");
        }
