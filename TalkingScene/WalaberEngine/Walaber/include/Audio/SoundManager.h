...
namespace Walaber
{
	...
	public:
        ...
        /*!
         Stops this sound, but only if it is a looping sound.
         */
        void stopIfLooping();
	...
	private:
        ...
        bool                    mIsMarkedForPaused; // need this variable since mChannel pause() can be called but a check for mChannel's state compared to Paused can be called before mChannel's state is actually set to paused by fmod
	};
	...
	public:
        ...
        /*!
         plays a random song from the group   
         @param group the group id of the group you want to get the filename for a random sound
         in that group
         @param volume number between 0 and 1 that adjusts the volume of the played sound
         @param pitch number between 0 and 1 that adjusts the pitch
         @return index of the song in the group that was played
         */
        int playSoundFromGroup( int group, float volume = 1.0f, float pitch = 1.0f );
        
        /*!
         returns the index in the group along with the filename in a single string
         @param group the group id of the group you want to get the filename for a random sound
                in that group
         @param soundID the index into the group for the specific sound to play
         */
        std::string getFilenameForSoundInGroup( const int group, const int soundID ) const;
		
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		int newSoundFromGroup( int group, CallbackPtr callback, const PropertyList& plist );
        ...
        /*! Dials up volume of required theme song up and dials down volumes of other themes. 
            @param groupID Identifier for which channel to dial up.
         */
        void switchThemeMusicTo( int groupID );
