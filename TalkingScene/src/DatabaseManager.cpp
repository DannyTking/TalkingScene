...
namespace Walaber 
{
    ...
    //------------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------------
    sqlite3* DatabaseManager::getDatabase(int databaseKey)
    {
        // Iterator position
	...
	void DatabaseManager::_readSQL(void* params)
    {
		...
		if (it != databaseMap.end())
		{
			...
			for (unsigned int i = 0; i < queries.size(); i++)
			{
				...
				if (query.length() > 5)
				{
					...
					if (gotError)
					{
						...
						RunSqlScriptErrorCallbackParameters params;
						params.errorCode = rc;
						params.sqlQuery = query;
						params.errorDescription = errDesc;
						params.outContinueExecuting = false;
						params.outIgnoreError = false;
						
						Logger::printf("Walaber", Logger::SV_DEBUG, "ERROR! [%s]", params.errorDescription.c_str());
						
						for(CallbackMultiMap::iterator it = errorCBrange.first; it != errorCBrange.second; ++it)
						{
							if(it->second)
							{
						...
						if (!params.outContinueExecuting)
						{
			...
			for(CallbackMultiMap::iterator it = range.first; it != range.second; )
			{
				if(it->second)
				{
				...
				std::map< std::string, CallbackPtr >::iterator toErase = it;
				++it;
			...
			for(CallbackMultiMap::iterator it = errorCBrange.first; it != errorCBrange.second; )
			{
				std::map< std::string, CallbackPtr >::iterator toErase = it;
				++it;
	...
	//------------------------------------------------------------------------------------------
	//------------------------------------------------------------------------------------------
	int DatabaseManager::getPlayerPlayedTime( int databaseKey, const std::string& tableName, const std::string& select, const std::string& where )
	{
